...
IFS=$'\n'
for streaminfo in $(${AWSCLI_PATH} logs describe-log-streams --output text --max-items 1000 --order-by LastEventTime --descending --log-group-name=$HEARTBEAT_GROUP --query "logStreams[?lastEventTimestamp > \`$(expr '(' $(date +%s) - 14400 ')' '*' 1000 ))\`][logStreamName, lastEventTimestamp]" | grep -v None); do
    aws_id=$(echo ${streaminfo} | cut -f1)
    last=$( expr $(echo ${streaminfo} | cut -f2) / 1000 )

    ${RIEMANNC_PATH} --service "aws.logs.describe-log-streams.heartbeat" --host ${aws_id} --ttl ${TTL} --metric_sint64 $(expr $(date +%s) - ${last} )
done
